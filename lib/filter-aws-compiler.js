        ...
        switch (node.operator) {
            ...
            default:
                ...
                throw JuttleErrors.compileError('RT-ADAPTER-UNSUPPORTED-FILTER',
                                                {proc: 'read aws', filter: "operator " + node.operator});
        }
    },
        ...
        switch (node.operator) {
            ...
            case '==':
                ...
                if (left !== "item" && left !== "product") {
                    throw JuttleErrors.compileError('RT-ADAPTER-UNSUPPORTED-FILTER',
                                                    {proc: 'read aws', filter: 'condition ' + left});
                }

                if (left === "product") {
                    if (! _.contains(this.supported_products, right)) {
                        throw JuttleErrors.compileError('RT-ADAPTER-UNSUPPORTED-FILTER',
                                                        {proc: 'read aws', filter: 'product ' + right});
                    }
                    return {
                        items: {},
                        products: [right]
                    ...
                    if (! this.cloudwatch) {
                        throw JuttleErrors.compileError('RT-ADAPTER-UNSUPPORTED-FILTER',
                                                        {proc: 'read aws', filter: 'specifying item filter when cloudwatch is disabled'});
                    }
                    ...
                    if (right.indexOf(":") === -1) {
                        throw JuttleErrors.compileError('RT-ADAPTER-UNSUPPORTED-FILTER',
                                                        {proc: 'read aws', filter: 'item value not having format <aws product>:<item name>'});
                    }
                    ...
                    if (! _.contains(this.supported_products, product)) {
                        throw JuttleErrors.compileError('RT-ADAPTER-UNSUPPORTED-FILTER',
                                                        {proc: 'read aws', filter: 'product ' + product});
                    }
