...
var Read = Juttle.proc.source.extend({
    ...
    initialize: function(options, params, location, program) {
        ...
        if (_.has(options, "cloudwatch")) {
            self.cloudwatch = options.cloudwatch;
        } else {
        ...
        if (params.filter_ast) {
            ...
            var compiler = new FilterAWSCompiler({
                ...
                supported_products: all_plugins
            });
        ...
        // Take the set of aws products and item types for the enabled
        // plugins and give them to Cloudwatch.
        self._plugins.forEach(function(plugin) {
            if (plugin.aws_product() !== "CloudWatch") {
